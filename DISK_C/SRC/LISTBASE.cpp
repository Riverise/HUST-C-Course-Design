...
void My_init(struct My_filenode **p0, struct My_filenode **ph, struct My_filenode **pc, struct My_filenode **p0_for_dir, struct My_filenode **ph_for_dir, struct My_filenode **pc_for_dir, struct My_filenode **p0_for_paste, struct My_filenode **ph_for_paste, struct My_filenode **pc_for_paste, struct My_vector_ **paste, struct My_vector_ **history, struct My_vector_ **currenthead, struct My_vector_ **search, char *sort_wayp)
{
	...
	*p0_for_dir = (struct My_filenode *)malloc(sizeof(struct My_filenode));
	*ph_for_dir = (struct My_filenode *)malloc(sizeof(struct My_filenode));
	*pc_for_dir = (struct My_filenode *)malloc(sizeof(struct My_filenode));
	...
	*search = (struct My_vector_ *)malloc(sizeof(struct My_vector_));
	*paste = (struct My_vector_ *)malloc(sizeof(struct My_vector_));
	*history = (struct My_vector_ *)malloc(sizeof(struct My_vector_));
	...
	(*p0_for_dir)->flag = IS_DIR;
	(*p0_for_dir)->flag += THIS_PC;
	strcpy((*p0_for_dir)->name, "cidiannao");
	(*p0_for_dir)->next = NULL;
	(*p0_for_dir)->father = NULL;
	(*p0_for_dir)->son_list_head = (*ph_for_dir);

	(*ph_for_dir)->flag = H_NODE;
	strcpy((*ph_for_dir)->name, "headNode");
	(*ph_for_dir)->next = (*pc_for_dir);
	(*ph_for_dir)->father = (*p0_for_dir);
	(*ph_for_dir)->son_list_head = NULL;

	(*pc_for_dir)->flag = IS_DIR;
	(*pc_for_dir)->flag += C_DISK;
	strcpy((*pc_for_dir)->name, "C:");
	(*pc_for_dir)->next = NULL;
	(*pc_for_dir)->father = (*p0_for_dir);
	(*pc_for_dir)->son_list_head = NULL;
...
{
	char typ[4]; //存储文件后缀名
	for (int i = 0; i < 12 && filename[i] != '\0' && filename[i] != '.'; i++)
		;
	...
	if (My_strcmp(typ, "TXT") == 0)
	{
		*flagp += TXT;
	}
	else if (My_strcmp(typ, "C") == 0)
	{
		*flagp += C;
	}
	else if (My_strcmp(typ, "CPP") == 0)
	{
		*flagp += CPP;
	}
	else if (My_strcmp(typ, "H") == 0)
	{
		*flagp += H;
	}
	else if (My_strcmp(typ, "OBJ") == 0)
	{
		*flagp += OBJ;
	}
	else if (My_strcmp(typ, "EXE") == 0)
	{
		*flagp += EXE;
	}
	else if (My_strcmp(typ, "JPG") == 0)
	{
		*flagp += JPG;
	}
	else if (My_strcmp(typ, "PNG") == 0)
	{
		*flagp += PNG;
	}
	else if (My_strcmp(typ, "DOC") == 0)
	{
		*flagp += DOC;
	}
	else if (My_strcmp(typ, "XLS") == 0)
	{
		*flagp += XLS;
	}
	else if (My_strcmp(typ, "PPT") == 0)
	{
		*flagp += PPT;
	}
	else if (My_strcmp(typ, "PDF") == 0)
	{
		*flagp += PDF;
	}
	else
...
struct My_filenode *end_of_list(struct My_filenode *theNode)
{
	...
	for (temp = theNode; temp->next != NULL; temp = temp->next)
		;
	return temp;
}
...
//{
//	int n = 0;
//	My_filenode* temp = headNode->father;
//	for (; temp->father->father!=NULL;temp=temp->father)
//	{
//		n += count_nodes(temp->son_list_head);
//	}
//	return n;
// }