 ...
 * 以及按位运算的函数
 *
 * > 1st 2022/3/14 创建相关定义和说明
 * > 2nd 2022/3/19 加入并修改一些按位操作函数
 * ************************/
...
/********************************
 * FUNCTION:gettype_dirdisplay
 * DESCRIPTION:获取Flag在头节点里设定的显示方式
 * INPUT:Flag变量
 * RETURN:int 型，表示显示方式
 * OTHERS:无
 ********************************/
int gettype_dirdisplay(Flag flag)
{
	char t;
	t = flag >> 6;
	return (t);
...
/********************************
 * FUNCTION:settype_dirdisplay
 * DESCRIPTION:设置头节点储存的文件显示方式变量
 * INPUT:头节点内量的地址，要设置的模式
 * RETURN:无
 * OTHERS:无
 ********************************/
void settype_dirdisplay(Flag *pflag, unsigned char display_type_mainfile)
{
	int i;
	display_type_mainfile = display_type_mainfile << 6;
	for (i = 0; i < 2; i++)
	{
		setonebit(pflag, i + 1, getonebit(display_type_mainfile, i + 1));
	}
}
